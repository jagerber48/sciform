...
* Previously when using ``bracket_unc=True`` with any exponent string
  ...
  format modes, the value and uncertainty are no longer wrapped in
  ...
  Note that the value and uncertainty are always wrapped in parentheses
  for the standard format mode and when using ``bracket_unc=False``,
  i.e. ``+/-`` style, formatting.
